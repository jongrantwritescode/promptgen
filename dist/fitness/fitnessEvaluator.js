...
export const llmFitnessEvaluator = {
    name: "llm",
    evaluate: async (prompt, testCases) => {
        let totalScore = 0;
        let validTests = 0;
        ...
        for (let i = 0; i < testCasesToUse.length; i++) {
            ...
            if (testCase) {
                console.log(`        🧪 Testing case ${i + 1}/${testCasesToUse.length}: "${testCase.input}" -> "${testCase.expectedOutput}"`);
                try {
                    const rubric = {
                        name: "accuracy",
                    ...
                    console.log(`        ✅ Test case ${i + 1} score: ${score.toFixed(3)}`);
                    totalScore += score;
                    validTests++;
                    ...
                    console.warn(`        ⚠️ Warning: Failed to evaluate test case ${i + 1} after all retries: ${error}`);
                }
            }
        }
        const finalScore = validTests > 0 ? totalScore / validTests : 0;
        console.log(`        🎯 LLM evaluation complete: ${finalScore.toFixed(3)} (${validTests} valid tests)`);
        return finalScore;
    },
};
export const heuristicFitnessEvaluator = {
    name: "heuristic",
    evaluate: async (prompt, testCases) => {
        let score = 0;
        ...
        const clarityScore = clarityWords.some((word) => prompt.toLowerCase().includes(word))
            ? 0.2
            : 0;
        score += clarityScore;
        // Structure indicators
        const structureScore = prompt.includes(":") || prompt.includes("•") || prompt.includes("-")
            ? 0.1
            : 0;
        score += structureScore;
        // Politeness (not too demanding)
        const politenessScore = prompt.toLowerCase().includes("please") ||
            prompt.toLowerCase().includes("kindly")
            ? 0.1
            : 0;
        score += politenessScore;
        // Specificity (mentions specific criteria)
        const specificityScore = prompt.toLowerCase().includes("classify") ||
        ...
        const formatScore = prompt.toLowerCase().includes("format") ||
...
//# sourceMappingURL=fitnessEvaluator.js.map