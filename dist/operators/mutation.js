    ...
    mutate: async (prompt) => {
        ...
        const mutatedWords = words.map((word) => {
            ...
            if (wordReplacements[cleanWord]) {
                ...
                if (replacement) {
                    return word.replace(new RegExp(cleanWord.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi"), replacement);
                }
            }
            return word;
        });
    ...
    mutate: async (prompt) => {
        ...
        for (const [original, replacements] of Object.entries(phraseReplacements)) {
            if (mutatedPrompt.toLowerCase().includes(original.toLowerCase())) {
                const replacement = replacements[Math.floor(Math.random() * replacements.length)];
                if (replacement) {
                    mutatedPrompt = mutatedPrompt.replace(new RegExp(original.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi"), replacement);
...
export const structureMutation = {
    name: "structure",
    mutate: async (prompt) => {
        const structures = [
            `Task: ${prompt}`,
